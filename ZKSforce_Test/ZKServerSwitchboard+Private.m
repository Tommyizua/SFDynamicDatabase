...
- (void) _returnResponseForConnection: (NSURLConnection *)connection {
	...
	YCElement *responseElement = nil;
	if ([data length] && [error code] != 401) {
		@try {
			responseElement = (YCElement *)[self _processHttpResponse:response data:data];
...
-(YCElement *)_processHttpResponse:(NSHTTPURLResponse *)resp data:(NSData *)responseData
{
	YCElement *root = [YCParser parseData:responseData];
	if (root == nil)	
	...
	YCElement *body = [root childElement:@"Body" ns:SOAP_NS];
	if (resp.statusCode == 500) 
    {
		...
		YCElement *fault = [body childElement:@"Fault" ns:SOAP_NS];
		if (fault == nil)
